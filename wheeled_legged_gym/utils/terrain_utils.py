...
def random_uniform_terrain(terrain, min_height, max_height, step=1, downsampled_scale=None):
    ...
    Parameters
        ...
        downsampled_scale (float): distance between two randomly sampled points
                                   (must be larger or equal to terrain.horizontal_scale)
    """
    ...
    step = max(1, int(step / terrain.vertical_scale))  # avoid zero step

    heights_range = np.arange(min_height, max_height + step, step)
    downsampled_width = int(terrain.width * terrain.horizontal_scale / downsampled_scale)
    downsampled_length = int(terrain.length * terrain.horizontal_scale / downsampled_scale)

    height_field_downsampled = np.random.choice(heights_range, (downsampled_width, downsampled_length))

    # define grid for interpolator
    x = np.linspace(0, terrain.width * terrain.horizontal_scale, downsampled_width)
    y = np.linspace(0, terrain.length * terrain.horizontal_scale, downsampled_length)

    interpolator = RegularGridInterpolator(
        (x, y), height_field_downsampled, method='linear', bounds_error=False, fill_value=0
    )

    # generate upsampled query grid
    ...
    z_upsampled = interpolator(query_points).reshape((terrain.width, terrain.length))
    z_upsampled = np.rint(z_upsampled)
