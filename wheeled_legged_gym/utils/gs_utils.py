...
def gs_rand_uniform_float(lower, upper, shape, device, dtype=torch.float32):
    if isinstance(shape, int):
        shape = (shape,)
    return (upper - lower) * torch.rand(size=shape, device=device, dtype=dtype) + lower


def euler_to_quat_radian(euler_xyz):
    if isinstance(euler_xyz, tuple):
        euler_xyz = np.array(euler_xyz)
    if isinstance(euler_xyz, list):
        euler_xyz = np.array(euler_xyz)
    return xyz_to_quat(euler_xyz, degrees=False)
...
def gs_rand_float(lower, upper, shape, device, dtype=torch.float32):
    return (upper - lower) * torch.rand(size=shape, device=device, dtype=dtype) + lower
